...
function normalizePerson(posts, lead = {}) {
  ...
  const apolloPerson = lead.raw_data?.apollo?.bulkMatch
    || lead.raw_data?.apollo?.searchPerson
    || {};
  return {
    first_name: clean(lead.first_name || apolloPerson.first_name),
    last_name: clean(lead.last_name || apolloPerson.last_name),
    headline: clean(profile.headline || apolloPerson.headline || author.headline),
    title: clean(lead.title || apolloPerson.title || profile.title),
    location: clean(profile.location || lead.location || author.location),
    linkedin_url: clean(lead.linkedin_url || apolloPerson.linkedin_url || author.profile_url),
