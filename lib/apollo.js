...
// Curated industry suggestions for the UI. Apollo's People Search API documents
// `q_keywords` as free-form, so custom values remain supported and this list is
...
async function searchPeople(filters, page = 1, perPage = 100) {
  ...
  if (filters.industry) {
    // The People Search endpoint documents q_keywords. The similarly named
    // q_organization_keyword_tags[] belongs to Organization Search instead.
    query.q_keywords = filters.industry;
  }
...
function compactOrganizationData(organization = {}) {
  const technologies = organization.technology_names
    || organization.current_technologies?.map(item => item?.name).filter(Boolean)
...
function buildPersonalizationProfile(person = {}, organization = {}, enrichedAt = new Date().toISOString()) {
  const company = compactOrganizationData(organization);
  const employment = Array.isArray(person.employment_history) ? person.employment_history : [];
  const evidence = [];
  if (company.latest_funding_stage) evidence.push({ fact: `${company.name || 'The company'} most recently raised a ${company.latest_funding_stage} round`, source: 'Apollo company enrichment' });
  if (company.estimated_num_employees) evidence.push({ fact: `${company.name || 'The company'} has approximately ${company.estimated_num_employees} employees`, source: 'Apollo company enrichment' });
  if (company.industry) evidence.push({ fact: `${company.name || 'The company'} operates in ${company.industry}`, source: 'Apollo company enrichment' });

  return {
    version: 1,
    enriched_at: enrichedAt,
    person: {
      name: clean(person.name || `${clean(person.first_name)} ${clean(person.last_name)}`),
      ...
      departments: uniqueStrings(person.departments, 6),
      location: uniqueStrings([person.city, person.state, person.country]).join(', '),
      linkedin_url: clean(person.linkedin_url),
      relevant_experience: employment.filter(item => item?.organization_name || item?.title).slice(0, 3).map(item => ({
        organization: clean(item.organization_name), title: clean(item.title), current: Boolean(item.current),
      })),
    },
    company: {
      name: company.name, domain: company.domain, website_url: company.website_url,
      linkedin_url: company.linkedin_url,
      industry: company.industry, employee_count: company.estimated_num_employees,
      location: uniqueStrings([company.city, company.state, company.country]).join(', '),
      description: company.short_description, founded_year: organization.founded_year || null,
      funding_stage: company.latest_funding_stage, funding_date: company.latest_funding_round_date,
      technologies: uniqueStrings(company.technologies, 10), keywords: uniqueStrings(organization.keywords, 12),
    },
    email_context: evidence.slice(0, 2),
  };
}
...
function normalizeApolloLead(person = {}, sourcePayload = {}) {
  ...
  const suppliedName = clean(person.name || person.full_name);
  const name = firstName && lastName
    ? `${firstName} ${lastName}`
    : suppliedName || `${firstName} ${lastName}`;
  const linkedinUrl = clean(person.linkedin_url || person.linkedinUrl || person.profile_url);
  const companyDomain = organizationDomain(organization, person);
  const phone = clean(
    person.phone
    || person.phone_number
  ...
  return {
    ...
    personalization_profile: buildPersonalizationProfile(person, organization),
    raw_data: {
      apollo: {
        searchPerson: person,
...
function extractBulkMatchUpdates(payload = {}) {
  ...
  return matches.map(rawMatch => {
    const match = rawMatch?.person || rawMatch || {};
    const organization = match.organization || match.account || {};
    return {
      personId: apolloPersonId(match),
      linkedinUrl: clean(match.linkedin_url || match.linkedinUrl || match.profile_url),
      companyLinkedinUrl: clean(
        organization.linkedin_url
        || organization.linkedinUrl
        || match.organization_linkedin_url,
      ),
      companyDomain: organizationDomain(organization, match),
      match,
    };
  }).filter(update => update.personId);
}
...
function mergeNonEmptyValues(existing = {}, incoming = {}) {
  const merged = { ...(existing || {}) };
  for (const [key, value] of Object.entries(incoming || {})) {
    if (hasUsableValue(value)) merged[key] = value;
  }
  return merged;
}

function mergeApolloPersonalizationProfile(existing = {}, incoming = {}) {
  const current = existing || {};
  const next = incoming || {};
  const profile = {
    ...current,
    person: mergeNonEmptyValues(current.person, next.person),
    company: mergeNonEmptyValues(current.company, next.company),
  };

  if (!hasUsableValue(current.email_context) && hasUsableValue(next.email_context)) {
    profile.email_context = next.email_context;
  }
  if (!hasUsableValue(current.version) && hasUsableValue(next.version)) profile.version = next.version;
  if (!hasUsableValue(current.enriched_at) && hasUsableValue(next.enriched_at)) profile.enriched_at = next.enriched_at;
  return profile;
}

function buildApolloMatchProfilePatch(lead = {}, matchUpdate = {}) {
  const normalized = normalizeApolloLead(matchUpdate.match || {});
  const patch = {
    company_data: mergeNonEmptyValues(lead.company_data, normalized.company_data),
    personalization_profile: mergeApolloPersonalizationProfile(
      lead.personalization_profile,
      normalized.personalization_profile,
    ),
  };

  if (normalized.first_name) patch.first_name = normalized.first_name;
  if (normalized.last_name) patch.last_name = normalized.last_name;
  if (normalized.full_name && normalized.full_name !== 'Apollo lead') patch.full_name = normalized.full_name;
  if (matchUpdate.linkedinUrl) patch.linkedin_url = matchUpdate.linkedinUrl;
  if (matchUpdate.companyDomain) patch.company_domain = matchUpdate.companyDomain;
  return patch;
}
...
async function requestBulkEnrichment(people, importRunId) {
  ...
  if (!webhookUrl) {
    const error = new Error('APOLLO_WEBHOOK_URL or APP_PUBLIC_URL is required for Apollo waterfall email enrichment');
...
async function pollWebhookResult(requestId) {
  ...
  if (process.env.APOLLO_POLL_WEBHOOK_RESULTS !== 'true') {
    debug('poll_webhook_result_skipped', {
      requestIdSuffix: requestId.slice(-8),
      reason: 'Webhook delivery is the configured enrichment completion path',
    });
    return null;
  }
...
function extractWebhookUpdates(payload = {}) {
  ...
  walk(payload, node => {
    ...
    const organization = node.organization || node.account || {};
    const linkedinUrl = clean(node.linkedin_url || node.linkedinUrl || node.profile_url);
    const companyLinkedinUrl = clean(
      organization.linkedin_url
      || organization.linkedinUrl
      || node.organization_linkedin_url,
    );
    const companyDomain = organizationDomain(organization, node);
    ...
    if (isWebhookResult && (personId || email || phone || linkedinUrl)) {
      updates.push({
        personId,
        email,
        phone,
  ...
  for (const update of updates) {
    const key = update.personId || update.email || update.phone || update.linkedinUrl;
    if (!key) continue;
