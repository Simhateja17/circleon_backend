...
async function generateEmailSequence({ steps, lead, companyData, agentConfig, previousEmails = [], validationErrors = [] }) {
  ...
  const result = await generateJson({
    ...
    systemInstruction: [
      ...
      'Keep the first touch to 50-70 words, ordinary follow-ups to 20-35 words, and a breakup/final note to 20-30 words.',
      ...
      'Avoid em dashes, fake praise, hype, guarantees, and US-specific claims.',
      ...(validationErrors.length ? [`A previous sequence failed validation. Correct these issues: ${validationErrors.join('; ')}`] : []),
    ].join('\n'),
    prompt: JSON.stringify({
