...
function leadSummary(lead = {}) {
  ...
  return {
    ...
    first_name: lead.first_name || person.first_name || null,
    title: lead.title,
    company_name: lead.company_name,
    location: lead.location,
    linkedin_url: lead.linkedin_url || null,
    person: {
      first_name: person.first_name || lead.first_name || null,
      last_name: person.last_name || lead.last_name || null,
...
async function generateEmailStep({ step, lead, companyData, agentConfig, previousEmails = [], icebreaker = '', validationErrors = [] }) {
  ...
  const wordLimit = isFirstTouch ? '50-70 words' : isBreakup ? '20-30 words' : '20-35 words';
  const result = await generateJson({
    temperature: 0.7,
    systemInstruction: [
...
function sequenceWordTarget(step = {}) {
  const stepNumber = Number(step.step_number || 1);
  const isBreakup = String(step.name || '').toLowerCase().includes('break');
  return {
    subjectMin: 4,
    subjectMax: 6,
    bodyMin: stepNumber === 1 ? 55 : isBreakup ? 22 : 24,
    bodyMax: stepNumber === 1 ? 62 : isBreakup ? 27 : 30,
  };
}
...
async function generateEmailSequence({ steps, lead, companyData, agentConfig, previousEmails = [], validationErrors = [] }) {
  ...
  const wordTargets = requestedSteps.map(step => {
    const target = sequenceWordTarget(step);
    return `Step ${step.step_number}: subject ${target.subjectMin}-${target.subjectMax} words; body ${target.bodyMin}-${target.bodyMax} words`;
  }).join('. ');
  ...
  const result = await generateJson({
    ...
    systemInstruction: [
      ...
      'Subjects must be 4-6 words. Count words before returning each subject.',
      `Use these safe word targets, which are intentionally inside the application limits: ${wordTargets}.`,
      'Count body words before returning. If a draft is too long, remove a sentence or shorten it; never exceed the safe maximum to preserve a CTA.',
      ...
      'Avoid em dashes, fake praise, hype, guarantees, and US-specific claims.',
      ...(repairPass ? [
        'This is a repair pass. Rewrite only the requested steps that failed validation, preserve the sequence purpose, and obey every safe word target above.',
    ...
    prompt: JSON.stringify({
      requested_steps: requestedSteps.map(step => ({
        ...
        sequence_instruction: step.ai_instruction || '',
        word_target: sequenceWordTarget(step),
      })),
      lead: leadSummary({ ...lead, company_data: companyData || lead?.company_data }),
      agent: agentSummary(agentConfig),
