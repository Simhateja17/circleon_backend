...
const { getBilling, isActiveSubscription } = require('./billing');
const { buildDisclosurePrompt } = require('./callingPrompt');
const { createRetellAgent, createRetellLlm } = require('./retell');
const { generateJson, getAiModelName, getAiProvider } = require('./gemini');
...
async function generateLaunchPlaybook({ workspace, agentConfig }) {
  try {
    const playbook = await generateJson({
      temperature: 0.3,
      systemInstruction: [
        'You are designing a launch-ready outbound calling playbook for a sales agent.',
        'Return only valid JSON matching the requested fields. Do not include markdown, code fences, or commentary.',
        'The response must be directly usable to provision a Retell LLM after a compliance wrapper is added by the backend.',
      ].join('\n'),
      prompt: buildGeminiPrompt({ workspace, agentConfig }),
    });
    return { playbook, model: getAiModelName() };
  } catch (error) {
    error.code = error.code || `${getAiProvider().toUpperCase().replace(/-/g, '_')}_REQUEST_FAILED`;
    throw error;
  }
}
...
async function processAgentLaunchJob(jobId) {
  ...
  const billing = await getBilling(supabase, job.workspace_id);
  if (!isActiveSubscription(billing)) throw new Error('Active paid subscription required before provisioning an agent');
