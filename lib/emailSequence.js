const { generateEmailSequence, getAiModelName, getAiProvider } = require('./gemini');
const { appendSignature, validateSequenceDrafts } = require('./emailValidation');

function logEmail(event, details = {}, level = 'info') {
...
function effectiveAgentConfig(agentConfig, campaign) {
  ...
  const snapshotWriting = snapshot.writing_config || {};
  const briefWriting = brief.writing_config || {};
  const writingConfig = {
    ...snapshotWriting,
    ...briefWriting,
    angle: brief.campaign_angle || briefWriting.angle || snapshotWriting.angle || '',
    tone: brief.tone || briefWriting.tone || snapshotWriting.tone || agentConfig.tone || '',
    cta: brief.cta || briefWriting.cta || snapshotWriting.cta || agentConfig.booking_link || '',
    proof: brief.proof || briefWriting.proof || snapshotWriting.proof || '',
    language: brief.language || briefWriting.language || snapshotWriting.language || 'English',
    sector: brief.sector || briefWriting.sector || snapshotWriting.sector || '',
    signature: brief.signature || briefWriting.signature || snapshotWriting.signature || '',
  };
  return {
    ...agentConfig,
    ...snapshot,
...
function researchContext(lead = {}) {
  const profile = lead.personalization_profile || {};
  const evidence = (profile.evidence || profile.email_context || [])
    .filter(item => item && (item.claim || item.fact || item.excerpt))
    .slice(0, 8);
  const inferredScore = evidence.length ? 1 : 0;
  return {
    score: Number(profile.personalization_score ?? inferredScore),
    status: lead.research_status || (profile.source === 'apollo_fallback' ? 'fallback' : 'not_started'),
    sourceFieldsUsed: profile.source_fields_used || [],
    evidence,
  };
}
...
function asGeneratedPreviousEmail(email) {
  return { step_number: email.step_number, subject: email.subject, body: email.body };
}
...
async function generateLeadSequence({ supabase, workspaceId, campaign, agentConfig, lead, forceFromStep = null }) {
  ...
  const research = researchContext(lead);
  const aiProvider = research.score > 0 ? getAiProvider() : null;
  const aiModel = research.score > 0 ? getAiModelName() : null;
  logEmail('email_generation_started', {
    workspaceId,
    ...
    steps: stepsToGenerate.map(step => step.step_number),
    provider: aiProvider,
    model: aiModel,
  });
  if (research.score <= 0) {
    logEmail('email_generation_skipped_insufficient_research', {
      workspaceId,
      campaignId: campaign.id,
      leadId: lead.id,
      researchScore: research.score,
      researchStatus: research.status,
      skippedSteps: stepsToGenerate.map(step => step.step_number),
    }, 'warn');
    return {
      generated,
      skipped: skipped + stepsToGenerate.length,
      skippedReason: 'insufficient_research',
      latestMessage,
    };
  }

  let generatedEmails = await generateEmailSequence({
  ...
  let validation = validateSequenceDrafts({
    emails: generatedEmails,
    steps: stepsToGenerate,
    evidence: research.evidence,
    personalizationScore: research.score,
  });
  let validationRegenerations = 0;
  if (!validation.valid) {
    logEmail('email_generation_validation_failed', {
      workspaceId,
      campaignId: campaign.id,
      leadId: lead.id,
    ...
    validationRegenerations = 1;
    const invalidStepNumbers = new Set(
      validation.results.filter(result => !result.valid).map(result => Number(result.step_number)),
    );
    const repairSteps = stepsToGenerate.filter(step => invalidStepNumbers.has(Number(step.step_number)));
    const preservedEmails = generatedEmails.filter(email => !invalidStepNumbers.has(Number(email.step_number)));
    const repairedEmails = await generateEmailSequence({
      steps: repairSteps,
      lead,
      companyData,
      agentConfig,
      previousEmails: [...previousEmails, ...preservedEmails.map(asGeneratedPreviousEmail)],
      validationErrors: validation.errors,
    });
    const repairedByStep = new Map(repairedEmails.map(email => [Number(email.step_number), email]));
    const originalByStep = new Map(generatedEmails.map(email => [Number(email.step_number), email]));
    generatedEmails = stepsToGenerate.map(step => repairedByStep.get(Number(step.step_number)) || originalByStep.get(Number(step.step_number)));
    validation = validateSequenceDrafts({
      emails: generatedEmails,
      steps: stepsToGenerate,
      evidence: research.evidence,
      personalizationScore: research.score,
  ...
  if (!validation.valid) {
    logEmail('email_generation_validation_failed_final', {
      workspaceId,
      campaignId: campaign.id,
      leadId: lead.id,
      errors: validation.errors,
    }, 'error');
    throw new Error(`Generated email failed validation: ${validation.errors.join('; ')}`);
  }

  const signature = agentConfig.writing_config?.signature || '';
  ...
  for (const step of stepsToGenerate) {
    ...
    const body = appendSignature(email.body, signature);
    const generationMeta = {
      ...
      research: {
        ...
        source_fields_used: research.sourceFieldsUsed,
        evidence: research.evidence.map(item => ({
          id: item.id,
          claim: item.claim || item.fact,
      ...
      validation: {
        ...
        regenerated_once: Boolean(validationRegenerations),
    ...
    const rawPayload = {
      ...
      generated_by: getAiProvider(),
      generated_at: generationMeta.generated_at,
  ...
  logEmail('email_generation_completed', {
    workspaceId,
    campaignId: campaign.id,
    leadId: lead.id,
    generated,
    skipped,
    researchScore: research.score,
    validationRegenerated: Boolean(validationRegenerations),
  });

  return { generated, skipped, latestMessage, researchScore: research.score };
}
...
async function preGenerateSequence({ supabase, workspaceId, campaignId, leadIds = [], concurrency = 1, onProgress = null }) {
  ...
  async function generateNextLead() {
    while (nextLeadIndex < leads.length) {
      ...
      let leadResult = null;
      try {
        leadResult = await generateLeadSequence({ supabase, workspaceId, campaign, agentConfig, lead });
        generated += leadResult.generated;
        skipped += leadResult.skipped;
      } catch (error) {
        errors.push({ lead_id: lead.id, message: error.message || 'Generation failed' });
        logEmail('email_generation_failed', {
          workspaceId,
          campaignId,
          leadId: lead.id,
      ...
      if (onProgress) {
        await onProgress({
          ...
          leadId: lead.id,
          skipped_reason: leadResult?.skippedReason || null,
        });
      }
    }
  }
