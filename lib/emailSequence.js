...
async function generateLeadSequence({ supabase, workspaceId, campaign, agentConfig, lead, forceFromStep = null }) {
  ...
  let icebreaker = existingByStep.get(1)?.raw_payload?.icebreaker || '';
  let generated = 0;
  let skipped = 0;
  ...
  for (const step of steps) {
    ...
    if (step.step_number === 1 && !icebreaker) {
      icebreaker = await generateIcebreaker({ lead, companyData, agentConfig });
    }
    const email = await generateEmailStep({
      ...
      agentConfig,
      previousEmails,
      icebreaker: step.step_number === 1 ? icebreaker : '',
    });
    const rawPayload = {
...
async function preGenerateSequence({ supabase, workspaceId, campaignId, leadIds = [], concurrency = 2, onProgress = null }) {
