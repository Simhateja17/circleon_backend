...
async function getCampaignLeads(supabase, workspaceId, campaign) {
  if (campaign.import_run_id) {
    const { data, error } = await supabase
      .from('leads')
      .select('*')
      .eq('workspace_id', workspaceId)
      .eq('import_run_id', campaign.import_run_id)
      .not('email', 'is', null)
...
async function attachLeadsToCampaign(supabase, workspaceId, campaignId, leadIds = []) {
...
async function preGenerateStep1({ supabase, workspaceId, campaignId, leadIds = [] }) {
  await attachLeadsToCampaign(supabase, workspaceId, campaignId, leadIds);
  ...
  const leads = await getCampaignLeads(supabase, workspaceId, campaign);
  if (!leads.length) {
