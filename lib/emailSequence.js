...
async function regenerateStep1Message({ supabase, workspaceId, campaignId, messageId }) {
  const [campaign, agentConfig] = await Promise.all([
    getCampaign(supabase, workspaceId, campaignId),
    getAgentConfig(supabase, workspaceId),
  ]);
  ...
  if (!message || message.direction !== 'outbound' || message.sequence_step !== 1 || !message.leads?.email) {
    throw new Error('Only an unsent first-step campaign email can be regenerated');
  }
  if (!['draft', 'approved'].includes(message.status)) {
    throw new Error(`This email cannot be regenerated because it is ${message.status}`);
  }

  const step = firstStep(campaign);
  const companyData = message.leads.company_data || {};
  const icebreaker = await generateIcebreaker({ lead: message.leads, companyData, agentConfig });
  const email = await generateEmailStep({ step, lead: message.leads, companyData, agentConfig, icebreaker });
  const { data: updated, error: updateError } = await supabase
    ...
    .update({
      ...
      raw_payload: { ...(message.raw_payload || {}), icebreaker, generated_by: 'gemini', regenerated_at: new Date().toISOString() },
    })
    .eq('id', message.id)
    .select('*')
    .single();
