...
function bodyLimits(step = {}) {
  if (Number(step.step_number || 1) === 1) return { min: 50, max: 70 };
  if (String(step.name || '').toLowerCase().includes('break')) return { min: 20, max: 30 };
...
function validateEmailDraft({ email = {}, step = {}, evidence = [], personalizationScore = 0 }) {
  ...
  if (bodyWords < limits.min || bodyWords > limits.max) errors.push(`Step ${step.step_number || 1} body must be ${limits.min}-${limits.max} words, got ${bodyWords}`);
  ...
  return { valid: errors.length === 0, errors, subjectWords, bodyWords, limits };
}
